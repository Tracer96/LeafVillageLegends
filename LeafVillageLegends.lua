...
local GEAR_LEFT_COLUMN_SLOTS = {
  "HeadSlot", "NeckSlot", "ShoulderSlot", "BackSlot", "ChestSlot",
  "WristSlot", "HandsSlot", "Finger0Slot", "Trinket0Slot",
}

local GEAR_RIGHT_COLUMN_SLOTS = {
  "WaistSlot", "LegsSlot", "FeetSlot", "Finger1Slot", "Trinket1Slot",
  "MainHandSlot", "SecondaryHandSlot", "RangedSlot",
}
...
function LeafVE:BroadcastLeaderboardData(includeKnownCache)
  ...
  if includeKnownCache then
    ...
    for name, _ in pairs(seen) do
      local lbase = LeafVE_DB.alltime[name] or LeafVE_DB.lboard.alltime[name]
      ...
      self:UpsertSharedLeaderboard(name, {L = lL, G = lG, S = lS}, {L = wL, G = wG, S = wS}, wk)
    end
  else
    ...
    self:UpsertSharedLeaderboard(me, {L = lL, G = lG, S = lS}, {L = wL, G = wG, S = wS}, wk)
  end
...
local function CreateGearSlotEntry(parent)
  ...
  slotText:SetPoint("TOPLEFT", icon, "TOPRIGHT", 6, -2)
  slotText:SetWidth(152)
...
local function LayoutGearColumn(columnFrame, entries, slotList)
  local spacing = 10
  local count = table.getn(slotList)
  if count <= 0 then return end
  local usable = columnFrame:GetHeight()
  if usable < 260 and columnFrame:GetParent() and columnFrame:GetParent().GetHeight then
    usable = columnFrame:GetParent():GetHeight() - 80
  end
  local minSpacing = 4
  local minEntryH = 52
  local minUsable = (count * minEntryH) + ((count - 1) * minSpacing)
  if usable < minUsable then
    usable = minUsable
  end
  local entryH = (usable - ((count - 1) * spacing)) / count
  if entryH < minEntryH then
    entryH = minEntryH
    if count > 1 then
      spacing = (usable - (entryH * count)) / (count - 1)
  ...
  for i = 1, count do
    ...
    yOffset = yOffset + entryH + spacing
  end
end
...
function LeafVE.UI:CreateGearPopup()
  ...
  local leftColumn = CreateFrame("Frame", nil, popup)
  leftColumn:SetPoint("TOPLEFT", popup, "TOPLEFT", 14, -70)
  leftColumn:SetPoint("BOTTOMLEFT", popup, "BOTTOMLEFT", 14, 2)
  leftColumn:SetWidth(200)
  popup.leftColumn = leftColumn

  local rightColumn = CreateFrame("Frame", nil, popup)
  rightColumn:SetPoint("TOPRIGHT", popup, "TOPRIGHT", -14, -70)
  rightColumn:SetPoint("BOTTOMRIGHT", popup, "BOTTOMRIGHT", -14, 2)
  rightColumn:SetWidth(200)
  popup.rightColumn = rightColumn

  local centerPanel = CreateFrame("Frame", nil, popup)
  centerPanel:SetPoint("TOPLEFT", leftColumn, "TOPRIGHT", 8, 0)
  centerPanel:SetPoint("BOTTOMRIGHT", rightColumn, "BOTTOMLEFT", -8, 0)
  ...
  centerPanel:SetBackdropColor(0.02, 0.02, 0.05, 0.75)
  centerPanel:SetBackdropBorderColor(0.2, 0.2, 0.25, 0.7)
  popup.centerPanel = centerPanel

  local specIcon = centerPanel:CreateTexture(nil, "ARTWORK")
  specIcon:SetWidth(68)
  specIcon:SetHeight(68)
  specIcon:SetPoint("TOP", centerPanel, "TOP", 0, -14)
  popup.specIcon = specIcon

  local specText = centerPanel:CreateFontString(nil, "OVERLAY", "GameFontNormal")
  specText:SetPoint("TOP", specIcon, "BOTTOM", 0, -5)
  specText:SetTextColor(0.95, 0.95, 0.95)
  popup.specText = specText

  local statsTitle = centerPanel:CreateFontString(nil, "OVERLAY", "GameFontNormal")
  statsTitle:SetPoint("TOP", specText, "BOTTOM", 0, -8)
  statsTitle:SetText("|cFFFFD700Cached Stats|r")
  popup.statsTitle = statsTitle

  local statsText = centerPanel:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
  statsText:SetPoint("TOP", statsTitle, "BOTTOM", 0, -8)
  statsText:SetPoint("LEFT", centerPanel, "LEFT", 14, 0)
  statsText:SetPoint("RIGHT", centerPanel, "RIGHT", -14, 0)
  statsText:SetPoint("BOTTOM", centerPanel, "BOTTOM", 0, 12)
  statsText:SetJustifyH("LEFT")
  statsText:SetJustifyV("TOP")
  if statsText.SetFont then
    statsText:SetFont(STANDARD_TEXT_FONT, 12, "")
  end
  popup.statsText = statsText

  popup.leftEntries = {}
  for i = 1, table.getn(GEAR_LEFT_COLUMN_SLOTS) do
    popup.leftEntries[i] = CreateGearSlotEntry(leftColumn)
  end

  popup.rightEntries = {}
  for i = 1, table.getn(GEAR_RIGHT_COLUMN_SLOTS) do
    popup.rightEntries[i] = CreateGearSlotEntry(rightColumn)
  end
...
function LeafVE.UI:RefreshGearPopup(playerName, skipDeferredLayout)
  ...
  self.gearPopup.equippedText:SetText("Equipped: " .. equipped .. "/17")

  LayoutGearColumn(self.gearPopup.leftColumn, self.gearPopup.leftEntries, GEAR_LEFT_COLUMN_SLOTS)
  LayoutGearColumn(self.gearPopup.rightColumn, self.gearPopup.rightEntries, GEAR_RIGHT_COLUMN_SLOTS)

  for i = 1, table.getn(GEAR_LEFT_COLUMN_SLOTS) do
    local slotName = GEAR_LEFT_COLUMN_SLOTS[i]
    ...
    ApplySlotEntryData(self.gearPopup.leftEntries[i], slotName, itemId, iconTex, enchantId, unitToken)
    self.gearPopup.leftEntries[i]:Show()
  end

  for i = 1, table.getn(GEAR_RIGHT_COLUMN_SLOTS) do
    local slotName = GEAR_RIGHT_COLUMN_SLOTS[i]
    local itemId = slots and slots[slotName]
    local iconTex = snapshot and snapshot.icons and snapshot.icons[slotName]
    local enchantId = snapshot and snapshot.enchants and snapshot.enchants[slotName]
    if not iconTex and itemId then
    ...
    ApplySlotEntryData(self.gearPopup.rightEntries[i], slotName, itemId, iconTex, enchantId, unitToken)
    self.gearPopup.rightEntries[i]:Show()
  end
  ...
  local formattedStats = FormatGearStats(stats or {}, classTag, spec and spec.name)
  self.gearPopup.statsText:SetText(formattedStats or "|cFF888888No stats cached|r")
